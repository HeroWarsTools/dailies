...
(function () {
    ...
    async function init() {
        ...
        console.log('%c[HWH Daily Dashboard] Script initialized (v1.14.2).', 'color: #00bcd4; font-weight: bold;');
        ...
        const i18nData = {
            en: {
                DD_TITLE: 'Daily Dashboard',
                DD_DESC: 'Select conditions to validate for item exchange.',
                DD_LBL_ACT: 'Activity < 1750',
                DD_LBL_GLYPH: 'Glyph Enchanted',
                DD_LBL_REFILLS: 'Energy Refills (50)',
                DD_LBL_ENERGY: 'Campaign Energy <= 120',
                DD_LBL_AUTO_DASH: 'Auto Dashboard (Background Check)',
                DD_LBL_AUTO_EXEC: 'Auto Execute Sequence',
                DD_LBL_SHOW_BTN: 'Show "1750" Button',
                DD_BTN_RUN: 'Run Pre-Check',
                DD_BTN_FIX_GLYPH: 'Enchant Glyph',
                DD_BTN_FIX_REFILLS: 'Buy Energy (Loop)',
                DD_BTN_SPEND_ENERGY: 'Spend Energy (Monitor)',
                DD_BTN_EXCHANGE: 'Item Exchange',
                DD_BTN_SAVE_ACC: 'Save (Account)',
                DD_BTN_LOAD_DEF: 'Load Default',
                DD_BTN_SET_DEFAULT: 'Set as Default',
                DD_STATUS_OK: 'Condition met',
                DD_STATUS_NO: 'Condition NOT met',
                DD_ALL_MET: 'ALL CONDITIONS MET!',
                DD_NOT_MET: 'CONDITIONS NOT MET',
                DD_WAIT_RECHECK: 'Action done. Re-checking in 1s...',
                DD_STATUS_MONITORING: 'Monitoring energy... Re-check will run when stable.',
                DD_EXCHANGE_SUCCESS: 'Activity Received: ',
                DD_EXCHANGE_ERROR: 'Exchange Error: Check console',
                DD_NO_ITEMS: 'No suitable items found (>200 qty, <4 val)',
                DD_QUEST_SETTINGS: 'Dashboard Button Settings',
                DD_QUEST_CONFIG: 'Dashboard Button Configuration',
                DD_SEQ_TITLE: 'Auto-Execution Sequence',
                DD_SEQ_SPEND: '1 - Spend Energy',
                DD_SEQ_BUY: '2 - Buy Energy',
                DD_SEQ_GLYPH: '3 - Enchant a Glyph',
                DD_SEQ_GLYPH_TIMER: '3a - Glyph Timer (Safe Sync)',
                DD_SEQ_EXCH: '4 - Item Exchange',
                DD_WAIT_SYNC: 'Auto-Exec Paused: Waiting for Reset Sync'
            },
            ru: {
                DD_TITLE: 'Ежедневная панель',
                DD_DESC: 'Выберите условия для проверки обмена предметов.',
                DD_LBL_ACT: 'Активность < 1750',
                DD_LBL_GLYPH: 'Глиф зачарован',
                DD_LBL_REFILLS: 'Покупки энергии (50)',
                DD_LBL_ENERGY: 'Энергия кампании <= 120',
                DD_LBL_AUTO_DASH: 'Авто-проверка панели',
                DD_LBL_AUTO_EXEC: 'Авто-выполнение последовательности',
                DD_LBL_SHOW_BTN: 'Показать кнопку "1750"',
                DD_BTN_RUN: 'Запустить проверку',
                DD_BTN_FIX_GLYPH: 'Зачаровать глиф',
                DD_BTN_FIX_REFILLS: 'Купить энергию (Цикл)',
                DD_BTN_SPEND_ENERGY: 'Потратить энергию (Монитор)',
                DD_BTN_EXCHANGE: 'Обмен предметов',
                DD_BTN_SAVE_ACC: 'Сохранить (Аккаунт)',
                DD_BTN_LOAD_DEF: 'Сбросить (По умолчанию)',
                DD_BTN_SET_DEFAULT: 'Сделать по умолчанию',
                DD_STATUS_OK: 'Условие выполнено',
                DD_STATUS_NO: 'Условие НЕ выполнено',
                DD_ALL_MET: 'ВСЕ УСЛОВИЯ ВЫПОЛНЕНЫ!',
                DD_NOT_MET: 'УСЛОВИЯ НЕ ВЫПОЛНЕНЫ',
                DD_WAIT_RECHECK: 'Действие выполнено. Повторная проверка через 1 сек...',
                DD_STATUS_MONITORING: 'Отслеживание энергии... Повторная проверка после стабилизации.',
                DD_EXCHANGE_SUCCESS: 'Получено активности: ',
                DD_EXCHANGE_ERROR: 'Ошибка обмена: проверьте консоль',
                DD_NO_ITEMS: 'Подходящие предметы не найдены (>200 шт, <4 ценность)',
                DD_QUEST_SETTINGS: 'Настройки кнопки панели',
                DD_QUEST_CONFIG: 'Конфигурация кнопки панели',
                DD_SEQ_TITLE: 'Авто-выполнение (Последовательность)',
                DD_SEQ_SPEND: '1 - Потратить энергию',
                DD_SEQ_BUY: '2 - Купить энергию',
                DD_SEQ_GLYPH: '3 - Зачаровать глиф',
                DD_SEQ_GLYPH_TIMER: '3a - Таймер глифа (Синхронизация)',
                DD_SEQ_EXCH: '4 - Обмен предметов',
                DD_WAIT_SYNC: 'Авто-выполнение приостановлено: Ожидание синхронизации сброса'
            }
        };
        ...
        async function performItemExchange(neededActivity) {
            try {
                ...
                if (itemsInfo.length === 0) {
                    setProgress(I18N('DD_NO_ITEMS'), 4000);
                    return;
                }
                ...
                if (countToExchange > activeItem.count) {
                    setProgress(I18N('DD_EXCHANGE_ERROR'), 4000);
                    return;
                }
                ...
                setProgress(`${I18N('DD_EXCHANGE_SUCCESS')} ${response}`, 5000);
        ...
        function monitorEnergyAndRecheck(options) {
            setProgress(I18N('DD_STATUS_MONITORING'), 5000);
        ...
        async function runChecks(options, isManual = false) {
            ...
            if (options.autoExec) {
                ...
                if (options.seqBuy && !refillGreen) {
                    setProgress('Auto: ' + I18N('DD_BTN_FIX_REFILLS'), 2000);
                    performEnergyLoop().then(() => {
                        setProgress(I18N('DD_WAIT_RECHECK'), 1000);
                ...
                if (options.seqGlyph && !glyphGreen) {
                    setProgress('Auto: ' + I18N('DD_BTN_FIX_GLYPH'), 2000);
                ...
                if (options.seqExch && allMet && totalActivity < 1750) {
                    setProgress('Auto: ' + I18N('DD_BTN_EXCHANGE'), 2000);
            ...
            const contentHTML = `
                <div style="text-align: left; padding: 10px; font-size: 14px; color: #ddd;">
                    <h3 style="text-align: center; color: #fde5b6; margin-bottom: 15px;">${I18N('DD_TITLE')}</h3>
                    <div style="margin-bottom: 8px;">${activityGreen ? '✅' : '❌'} <b>${I18N('DD_LBL_ACT')}:</b> ${activityGreen ? I18N('DD_STATUS_OK') : I18N('DD_STATUS_NO')}</div>
                    <div style="margin-bottom: 8px;">${glyphGreen ? '✅' : '❌'} <b>${I18N('DD_LBL_GLYPH')}:</b> ${glyphGreen ? I18N('DD_STATUS_OK') : I18N('DD_STATUS_NO')}</div>
                    <div style="margin-bottom: 8px;">${refillGreen ? '✅' : '❌'} <b>${I18N('DD_LBL_REFILLS')}:</b> ${refillGreen ? I18N('DD_STATUS_OK') : I18N('DD_STATUS_NO')}</div>
                    <div style="margin-bottom: 15px;">${campaignEnergyGreen ? '✅' : '❌'} <b>${I18N('DD_LBL_ENERGY')}:</b> ${campaignEnergyGreen ? I18N('DD_STATUS_OK') : I18N('DD_STATUS_NO')}</div>
                    <hr style="border-color: #555;">
                    <div style="text-align: center; font-weight: bold; font-size: 16px; margin-top: 15px; color: ${allMet ? '#88cb13' : '#ff9800'};">
                        ${allMet ? I18N('DD_ALL_MET') : I18N('DD_NOT_MET')}
                    </div>
                </div>
            `;
            ...
            const popupButtons =[];
            if (!glyphGreen) {
                popupButtons.push({ msg: I18N('DD_BTN_FIX_GLYPH'), color: 'orange', result: async () => { await performGlyphFix(); setProgress(I18N('DD_WAIT_RECHECK'), 1000); await delay(1000); runChecks(options, true); } });
            }
            if (!refillGreen) {
                popupButtons.push({ msg: I18N('DD_BTN_FIX_REFILLS'), color: 'blue', result: async () => { await performEnergyLoop(); setProgress(I18N('DD_WAIT_RECHECK'), 1000); await delay(1000); runChecks(options, true); } });
            }

            if (!campaignEnergyGreen && typeof unsafeWindow.HWH_CampAuto_API?.run === 'function') {
                popupButtons.push({
                    msg: I18N('DD_BTN_SPEND_ENERGY'),
                    color: 'purple',
                    result: () => {
                        unsafeWindow.HWH_CampAuto_API.run();
                        monitorEnergyAndRecheck(options);
            ...
            if (allMet && totalActivity < 1750) {
                popupButtons.push({ msg: I18N('DD_BTN_EXCHANGE'), color: 'pink', result: () => performItemExchange(1750 - totalActivity) });
            }
        ...
        async function openDashboardPopup() {
            ...
            const contentHTML = `
                <div style="text-align: left; padding: 10px; font-size: 14px; color: #ddd;">
                    <h3 style="text-align: center; color: #fde5b6; margin-bottom: 15px;">${I18N('DD_TITLE')}</h3>
                    <div style="margin-bottom: 10px;"><input type="checkbox" id="db_act" ${opt.act ? 'checked' : ''}> <label for="db_act">${I18N('DD_LBL_ACT')}</label></div>
                    <div style="margin-bottom: 10px;"><input type="checkbox" id="db_gly" ${opt.gly ? 'checked' : ''}> <label for="db_gly">${I18N('DD_LBL_GLYPH')}</label></div>
                    <div style="margin-bottom: 10px;"><input type="checkbox" id="db_ref" ${opt.ref ? 'checked' : ''}> <label for="db_ref">${I18N('DD_LBL_REFILLS')}</label></div>
                    <div style="margin-bottom: 10px;"><input type="checkbox" id="db_nrg" ${opt.nrg ? 'checked' : ''}> <label for="db_nrg">${I18N('DD_LBL_ENERGY')}</label></div>
                    <hr style="border-color: #555;">
                    <div style="margin-bottom: 10px;"><input type="checkbox" id="db_auto_dash" ${opt.autoDash ? 'checked' : ''}> <label for="db_auto_dash"><b>${I18N('DD_LBL_AUTO_DASH')}</b></label></div>
                    <div style="margin-bottom: 10px;"><input type="checkbox" id="db_auto_exec" ${opt.autoExec ? 'checked' : ''}> <label for="db_auto_exec"><b>${I18N('DD_LBL_AUTO_EXEC')}</b></label></div>
                    <hr style="border-color: #555;">
                    <div style="text-align: center; color: #fde5b6; margin-bottom: 10px;"><b>${I18N('DD_SEQ_TITLE')}</b></div>
                    <div style="margin-bottom: 5px;"><input type="checkbox" id="db_seq_spend" ${opt.seqSpend ? 'checked' : ''}> <label for="db_seq_spend">${I18N('DD_SEQ_SPEND')}</label></div>
                    <div style="margin-bottom: 5px;"><input type="checkbox" id="db_seq_buy" ${opt.seqBuy ? 'checked' : ''}> <label for="db_seq_buy">${I18N('DD_SEQ_BUY')}</label></div>
                    <div style="margin-bottom: 5px;"><input type="checkbox" id="db_seq_glyph" ${opt.seqGlyph ? 'checked' : ''}> <label for="db_seq_glyph">${I18N('DD_SEQ_GLYPH')}</label></div>
                    <div style="margin-bottom: 5px; margin-left: 20px;"><input type="checkbox" id="db_seq_glyph_timer" ${opt.seqGlyphTimer ? 'checked' : ''}> <label for="db_seq_glyph_timer" style="color: #bbb; font-size: 13px;">${I18N('DD_SEQ_GLYPH_TIMER')}</label></div>
                    <div style="margin-bottom: 15px;"><input type="checkbox" id="db_seq_exch" ${opt.seqExch ? 'checked' : ''}> <label for="db_seq_exch">${I18N('DD_SEQ_EXCH')}</label></div>

                    <div style="display: flex; gap: 5px; margin-top: 15px;">
                        <div onclick="HWH_Dashboard_API.run()" class="PopUp_btnGap green" style="flex: 1; cursor: pointer; padding: 3px;"><div class="PopUp_btnPlate" style="font-weight: bold; padding: 5px 0;">${I18N('DD_BTN_RUN')}</div></div>
                        <div onclick="HWH_Dashboard_API.save()" class="PopUp_btnGap blue" style="flex: 1; cursor: pointer; padding: 3px;"><div class="PopUp_btnPlate" style="font-weight: bold; padding: 5px 0;">${I18N('DD_BTN_SAVE_ACC')}</div></div>
                    </div>
                    <div style="display: flex; gap: 5px; margin-top: 5px;">
                        <div onclick="HWH_Dashboard_API.setDefault()" class="PopUp_btnGap violet" style="flex: 1; cursor: pointer; padding: 3px;"><div class="PopUp_btnPlate" style="font-weight: bold; padding: 5px 0; font-size: 12px;">${I18N('DD_BTN_SET_DEFAULT')}</div></div>
                        <div onclick="HWH_Dashboard_API.loadDefault()" class="PopUp_btnGap red" style="flex: 1; cursor: pointer; padding: 3px;"><div class="PopUp_btnPlate" style="font-weight: bold; padding: 5px 0; font-size: 12px;">${I18N('DD_BTN_LOAD_DEF')}</div></div>
                    </div>
                </div>
            `;
        ...
        const syncQuestButton = () => {
            if (isButtonEnabled) {
                const customSideButton = {
                    ...
                    get title() { return I18N('DD_TITLE'); },
        ...
        HWHData.othersPopupButtons.push({
            msg: I18N('DD_QUEST_SETTINGS'),
            title: I18N('DD_QUEST_CONFIG'),
            color: 'violet',
            result: async function () {
                const { popup } = HWHFuncs;
                const contentHTML = `<div style="text-align: center; font-size: 14px; color: #fde5b6;">${I18N('DD_QUEST_CONFIG')}</div>`;
                const popupCheckboxes =[{ name: 'toggle_btn', label: I18N('DD_LBL_SHOW_BTN'), checked: isButtonEnabled }];
